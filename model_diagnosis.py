...
# Load the dataset
file_path = 'R:/CliniVision/CliniVision/train.csv'  # Update this path with your actual file location
data = pd.read_csv(file_path)

# One-hot encode symptom types
...
target = data['diagnosis']  # Adjust this line based on how your target variable is represented
...
model = Sequential([
    ...
    Dropout(0.5),
    Dense(1, activation='sigmoid')  # Adjust based on your target variable (e.g., number of classes)
])

# model.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])

# Train the model on the dataset
# odel.fit(X_train, y_train, validation_data=(X_val, y_val), epochs=10, batch_size=32)
